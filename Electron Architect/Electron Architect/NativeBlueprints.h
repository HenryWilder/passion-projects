#pragma once
#include "Blueprint.h"

// Built-in blueprints
inline Blueprint nativeBlueprints[] =
{
    Blueprint(
        "Switch",
        {
            NodeBP("In",        true, Gate::OR,  IVec2(0,0)* g_gridSize),
            NodeBP("Interface", true, Gate::OR,  IVec2(1,0)* g_gridSize),
            NodeBP("Out",       true, Gate::AND, IVec2(2,0)* g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::horizontal),
            WireBP(1, 2, ElbowConfig::horizontal),
        }),

    Blueprint(
        "Basic Circuit",
        {
            NodeBP("Power cell", false, Gate::BATTERY, IVec2(0,3)* g_gridSize),
            NodeBP("Switch state controller", true, Gate::OR, IVec2(2,0)* g_gridSize),
            NodeBP("Switch interface", false, Gate::AND, IVec2(2,1)* g_gridSize),
            NodeBP("Lamp", false, Gate::LED, 4, IVec2(4,3)* g_gridSize),
            NodeBP(false, Gate::OR, IVec2(2,5)* g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::horizontal),
            WireBP(1, 2, ElbowConfig::vertical),
            WireBP(2, 3, ElbowConfig::vertical),
            WireBP(3, 4, ElbowConfig::horizontal),
            WireBP(4, 0, ElbowConfig::vertical),
        }),

    Blueprint(
        "SR Latch",
        {
            NodeBP("Reset", true, Gate::OR, IVec2(0,0) * g_gridSize),
            NodeBP("Set", true, Gate::OR,  IVec2(0,1)* g_gridSize),
            NodeBP(false, Gate::OR,  IVec2(1,0) * g_gridSize),
            NodeBP(false, Gate::OR,  IVec2(1,1) * g_gridSize),
            NodeBP("Q", false, Gate::NOR, IVec2(2,0)* g_gridSize),
            NodeBP("Q'", true, Gate::NOR, IVec2(2,1)* g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::horizontal),
            WireBP(1, 3, ElbowConfig::horizontal),
            WireBP(2, 4, ElbowConfig::horizontal),
            WireBP(3, 5, ElbowConfig::horizontal),
            WireBP(4, 3, ElbowConfig::diagonalA),
            WireBP(5, 2, ElbowConfig::diagonalA),
        }),
        
    Blueprint(
        "Gated SR Latch",
        {
            NodeBP("Enable", true, Gate::OR, IVec2(0,1) * g_gridSize),

            NodeBP("Set", true, Gate::OR,  IVec2(0,0)* g_gridSize),

            NodeBP(false, Gate::OR,  IVec2(1,0) * g_gridSize),
            NodeBP("Reset (inverse of Set)", false, Gate::NOR, IVec2(1,1) * g_gridSize),

            NodeBP(false, Gate::AND, IVec2(2,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(2,1) * g_gridSize),

            NodeBP(false, Gate::OR,  IVec2(3,0) * g_gridSize),
            NodeBP(false, Gate::OR,  IVec2(3,1) * g_gridSize),

            NodeBP("Q", false, Gate::NOR, IVec2(4,0)* g_gridSize),
            NodeBP("Q (Out)", true, Gate::NOR, IVec2(4,1)* g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::diagonalA),
            WireBP(1, 3, ElbowConfig::diagonalA),
            WireBP(1, 4, ElbowConfig::diagonalA),
            WireBP(2, 4, ElbowConfig::diagonalA),
            WireBP(2, 5, ElbowConfig::diagonalA),
            WireBP(3, 5, ElbowConfig::diagonalA),
            WireBP(4, 6, ElbowConfig::diagonalA),
            WireBP(5, 7, ElbowConfig::diagonalA),
            WireBP(6, 8, ElbowConfig::diagonalA),
            WireBP(7, 9, ElbowConfig::diagonalA),
            WireBP(8, 7, ElbowConfig::diagonalA),
            WireBP(9, 6, ElbowConfig::diagonalA),
        }),

    Blueprint(
        "D Latch",
        {
            NodeBP("D", true, Gate::OR, IVec2(0,0) * g_gridSize),
            NodeBP("E", true, Gate::OR, IVec2(0,1) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,1) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(2,0) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(2,1) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(3,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(3,1) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(4,0) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(4,1) * g_gridSize),
            NodeBP("Q Out", true, Gate::OR, IVec2(5,0) * g_gridSize),
            NodeBP("Q'", true, Gate::OR, IVec2(5,1) * g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::horizontal),
            WireBP(1, 3, ElbowConfig::horizontal),
            WireBP(2, 4, ElbowConfig::horizontal),
            WireBP(3, 5, ElbowConfig::horizontal),
            WireBP(4, 6, ElbowConfig::horizontal),
            WireBP(5, 7, ElbowConfig::horizontal),
            WireBP(6, 8, ElbowConfig::horizontal),
            WireBP(7, 9, ElbowConfig::horizontal),
            WireBP(8, 10, ElbowConfig::horizontal),
            WireBP(9, 11, ElbowConfig::horizontal),

            WireBP(1, 2, ElbowConfig::diagonalB),
            WireBP(4, 3, ElbowConfig::diagonalB),

            WireBP(8, 7, ElbowConfig::diagonalB),
            WireBP(9, 6, ElbowConfig::diagonalB),
        }),

    Blueprint(
        "2-to-1 Bit Multiplexer (MUX)",
        {
            NodeBP("I0", true, Gate::OR, IVec2(0,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,0) * g_gridSize),
            NodeBP("I1", true, Gate::OR, IVec2(0,1) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,1) * g_gridSize),
            NodeBP("O", true, Gate::OR, IVec2(2,1) * g_gridSize),
            NodeBP("A", true, Gate::OR, IVec2(1,2)* g_gridSize),
            NodeBP("A'", false, Gate::NOR, IVec2(0,2)* g_gridSize),
        },
        {
            WireBP(0, 1, ElbowConfig::horizontal),
            WireBP(2, 3, ElbowConfig::horizontal),
            WireBP(1, 4, ElbowConfig::diagonalA),
            WireBP(3, 4, ElbowConfig::diagonalA),
            WireBP(5, 5, ElbowConfig::horizontal),
            WireBP(5, 1, ElbowConfig::vertical),
            WireBP(6, 3, ElbowConfig::diagonalB),
        }),

    Blueprint(
        "1-to-2 Bit Multiplexer (MUX)",
        {
            NodeBP("O0", true, Gate::OR, IVec2(2,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,0) * g_gridSize),
            NodeBP("O1", true, Gate::OR, IVec2(2,1) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,1) * g_gridSize),
            NodeBP("I", true, Gate::OR, IVec2(0,1) * g_gridSize),
            NodeBP("A", true, Gate::OR, IVec2(1,2)* g_gridSize),
            NodeBP("A'", false, Gate::NOR, IVec2(2,2)* g_gridSize),
        },
        {
            WireBP(1, 0, ElbowConfig::horizontal),
            WireBP(3, 2, ElbowConfig::horizontal),
            WireBP(4, 1, ElbowConfig::diagonalB),
            WireBP(4, 3, ElbowConfig::diagonalB),
            WireBP(5, 6, ElbowConfig::horizontal),
            WireBP(5, 1, ElbowConfig::vertical),
            WireBP(6, 3, ElbowConfig::diagonalB),
        }),

    Blueprint(
        "2-to-1 Byte Multiplexer (MUX)",
        {
            NodeBP("I0_1", true, Gate::OR, IVec2(0,000) * g_gridSize),
            NodeBP("I0_2", true, Gate::OR, IVec2(0,001) * g_gridSize),
            NodeBP("I0_3", true, Gate::OR, IVec2(0,002) * g_gridSize),
            NodeBP("I0_4", true, Gate::OR, IVec2(0,003) * g_gridSize),
            NodeBP("I0_5", true, Gate::OR, IVec2(0,004) * g_gridSize),
            NodeBP("I0_6", true, Gate::OR, IVec2(0,005) * g_gridSize),
            NodeBP("I0_7", true, Gate::OR, IVec2(0,006) * g_gridSize),
            NodeBP("I0_8", true, Gate::OR, IVec2(0,007) * g_gridSize),

            NodeBP(false, Gate::AND, IVec2(1,000) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,001) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,002) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,003) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,004) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,005) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,006) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,007) * g_gridSize),

            NodeBP("I1_1", true, Gate::OR, IVec2(0,010) * g_gridSize),
            NodeBP("I1_2", true, Gate::OR, IVec2(0,011) * g_gridSize),
            NodeBP("I1_3", true, Gate::OR, IVec2(0,012) * g_gridSize),
            NodeBP("I1_4", true, Gate::OR, IVec2(0,013) * g_gridSize),
            NodeBP("I1_5", true, Gate::OR, IVec2(0,014) * g_gridSize),
            NodeBP("I1_6", true, Gate::OR, IVec2(0,015) * g_gridSize),
            NodeBP("I1_7", true, Gate::OR, IVec2(0,016) * g_gridSize),
            NodeBP("I1_8", true, Gate::OR, IVec2(0,017) * g_gridSize),

            NodeBP(false, Gate::AND, IVec2(1,010) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,011) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,012) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,013) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,014) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,015) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,016) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,017) * g_gridSize),

            NodeBP("O_1", true, Gate::OR, IVec2(2,004) * g_gridSize),
            NodeBP("O_2", true, Gate::OR, IVec2(2,005) * g_gridSize),
            NodeBP("O_3", true, Gate::OR, IVec2(2,006) * g_gridSize),
            NodeBP("O_4", true, Gate::OR, IVec2(2,007) * g_gridSize),
            NodeBP("O_5", true, Gate::OR, IVec2(2,010) * g_gridSize),
            NodeBP("O_6", true, Gate::OR, IVec2(2,011) * g_gridSize),
            NodeBP("O_7", true, Gate::OR, IVec2(2,012) * g_gridSize),
            NodeBP("O_8", true, Gate::OR, IVec2(2,013) * g_gridSize),

            NodeBP("A", true, Gate::OR, IVec2(1,020)* g_gridSize),
            NodeBP("A'", false, Gate::NOR, IVec2(0,020)* g_gridSize),
        },
        {
            WireBP(000, 010, ElbowConfig::horizontal),
            WireBP(001, 011, ElbowConfig::horizontal),
            WireBP(002, 012, ElbowConfig::horizontal),
            WireBP(003, 013, ElbowConfig::horizontal),
            WireBP(004, 014, ElbowConfig::horizontal),
            WireBP(005, 015, ElbowConfig::horizontal),
            WireBP(006, 016, ElbowConfig::horizontal),
            WireBP(007, 017, ElbowConfig::horizontal),

            WireBP(020, 030, ElbowConfig::horizontal),
            WireBP(021, 031, ElbowConfig::horizontal),
            WireBP(022, 032, ElbowConfig::horizontal),
            WireBP(023, 033, ElbowConfig::horizontal),
            WireBP(024, 034, ElbowConfig::horizontal),
            WireBP(025, 035, ElbowConfig::horizontal),
            WireBP(026, 036, ElbowConfig::horizontal),
            WireBP(027, 037, ElbowConfig::horizontal),

            WireBP(010, 040, ElbowConfig::diagonalA),
            WireBP(011, 041, ElbowConfig::diagonalA),
            WireBP(012, 042, ElbowConfig::diagonalA),
            WireBP(013, 043, ElbowConfig::diagonalA),
            WireBP(014, 044, ElbowConfig::diagonalA),
            WireBP(015, 045, ElbowConfig::diagonalA),
            WireBP(016, 046, ElbowConfig::diagonalA),
            WireBP(017, 047, ElbowConfig::diagonalA),

            WireBP(030, 040, ElbowConfig::diagonalA),
            WireBP(031, 041, ElbowConfig::diagonalA),
            WireBP(032, 042, ElbowConfig::diagonalA),
            WireBP(033, 043, ElbowConfig::diagonalA),
            WireBP(034, 044, ElbowConfig::diagonalA),
            WireBP(035, 045, ElbowConfig::diagonalA),
            WireBP(036, 046, ElbowConfig::diagonalA),
            WireBP(037, 047, ElbowConfig::diagonalA),

            WireBP(050, 051, ElbowConfig::horizontal),

            WireBP(050, 010, ElbowConfig::vertical),
            WireBP(050, 011, ElbowConfig::vertical),
            WireBP(050, 012, ElbowConfig::vertical),
            WireBP(050, 013, ElbowConfig::vertical),
            WireBP(050, 014, ElbowConfig::vertical),
            WireBP(050, 015, ElbowConfig::vertical),
            WireBP(050, 016, ElbowConfig::vertical),
            WireBP(050, 017, ElbowConfig::vertical),

            WireBP(051, 030, ElbowConfig::diagonalB),
            WireBP(051, 031, ElbowConfig::diagonalB),
            WireBP(051, 032, ElbowConfig::diagonalB),
            WireBP(051, 033, ElbowConfig::diagonalB),
            WireBP(051, 034, ElbowConfig::diagonalB),
            WireBP(051, 035, ElbowConfig::diagonalB),
            WireBP(051, 036, ElbowConfig::diagonalB),
            WireBP(051, 037, ElbowConfig::diagonalB),
        }),

    Blueprint(
        "1-to-2 Byte Multiplexer (MUX)",
        {
            NodeBP("O0_1", true, Gate::OR, IVec2(2,000) * g_gridSize),
            NodeBP("O0_2", true, Gate::OR, IVec2(2,001) * g_gridSize),
            NodeBP("O0_3", true, Gate::OR, IVec2(2,002) * g_gridSize),
            NodeBP("O0_4", true, Gate::OR, IVec2(2,003) * g_gridSize),
            NodeBP("O0_5", true, Gate::OR, IVec2(2,004) * g_gridSize),
            NodeBP("O0_6", true, Gate::OR, IVec2(2,005) * g_gridSize),
            NodeBP("O0_7", true, Gate::OR, IVec2(2,006) * g_gridSize),
            NodeBP("O0_8", true, Gate::OR, IVec2(2,007) * g_gridSize),

            NodeBP(false, Gate::AND, IVec2(1,000) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,001) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,002) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,003) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,004) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,005) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,006) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,007) * g_gridSize),

            NodeBP("O1_1", true, Gate::OR, IVec2(2,010) * g_gridSize),
            NodeBP("O1_2", true, Gate::OR, IVec2(2,011) * g_gridSize),
            NodeBP("O1_3", true, Gate::OR, IVec2(2,012) * g_gridSize),
            NodeBP("O1_4", true, Gate::OR, IVec2(2,013) * g_gridSize),
            NodeBP("O1_5", true, Gate::OR, IVec2(2,014) * g_gridSize),
            NodeBP("O1_6", true, Gate::OR, IVec2(2,015) * g_gridSize),
            NodeBP("O1_7", true, Gate::OR, IVec2(2,016) * g_gridSize),
            NodeBP("O1_8", true, Gate::OR, IVec2(2,017) * g_gridSize),

            NodeBP(false, Gate::AND, IVec2(1,010) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,011) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,012) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,013) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,014) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,015) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,016) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,017) * g_gridSize),

            NodeBP("I_1", true, Gate::OR, IVec2(0,004) * g_gridSize),
            NodeBP("I_2", true, Gate::OR, IVec2(0,005) * g_gridSize),
            NodeBP("I_3", true, Gate::OR, IVec2(0,006) * g_gridSize),
            NodeBP("I_4", true, Gate::OR, IVec2(0,007) * g_gridSize),
            NodeBP("I_5", true, Gate::OR, IVec2(0,010) * g_gridSize),
            NodeBP("I_6", true, Gate::OR, IVec2(0,011) * g_gridSize),
            NodeBP("I_7", true, Gate::OR, IVec2(0,012) * g_gridSize),
            NodeBP("I_8", true, Gate::OR, IVec2(0,013) * g_gridSize),

            NodeBP("A", true, Gate::OR, IVec2(1,020)* g_gridSize),
            NodeBP("A'", false, Gate::NOR, IVec2(2,020)* g_gridSize),
        },
        {
            WireBP(010, 000, ElbowConfig::horizontal),
            WireBP(011, 001, ElbowConfig::horizontal),
            WireBP(012, 002, ElbowConfig::horizontal),
            WireBP(013, 003, ElbowConfig::horizontal),
            WireBP(014, 004, ElbowConfig::horizontal),
            WireBP(015, 005, ElbowConfig::horizontal),
            WireBP(016, 006, ElbowConfig::horizontal),
            WireBP(017, 007, ElbowConfig::horizontal),

            WireBP(030, 020, ElbowConfig::horizontal),
            WireBP(031, 021, ElbowConfig::horizontal),
            WireBP(032, 022, ElbowConfig::horizontal),
            WireBP(033, 023, ElbowConfig::horizontal),
            WireBP(034, 024, ElbowConfig::horizontal),
            WireBP(035, 025, ElbowConfig::horizontal),
            WireBP(036, 026, ElbowConfig::horizontal),
            WireBP(037, 027, ElbowConfig::horizontal),

            WireBP(040, 010, ElbowConfig::diagonalB),
            WireBP(041, 011, ElbowConfig::diagonalB),
            WireBP(042, 012, ElbowConfig::diagonalB),
            WireBP(043, 013, ElbowConfig::diagonalB),
            WireBP(044, 014, ElbowConfig::diagonalB),
            WireBP(045, 015, ElbowConfig::diagonalB),
            WireBP(046, 016, ElbowConfig::diagonalB),
            WireBP(047, 017, ElbowConfig::diagonalB),

            WireBP(040, 030, ElbowConfig::diagonalB),
            WireBP(041, 031, ElbowConfig::diagonalB),
            WireBP(042, 032, ElbowConfig::diagonalB),
            WireBP(043, 033, ElbowConfig::diagonalB),
            WireBP(044, 034, ElbowConfig::diagonalB),
            WireBP(045, 035, ElbowConfig::diagonalB),
            WireBP(046, 036, ElbowConfig::diagonalB),
            WireBP(047, 037, ElbowConfig::diagonalB),

            WireBP(050, 051, ElbowConfig::horizontal),

            WireBP(050, 010, ElbowConfig::vertical),
            WireBP(050, 011, ElbowConfig::vertical),
            WireBP(050, 012, ElbowConfig::vertical),
            WireBP(050, 013, ElbowConfig::vertical),
            WireBP(050, 014, ElbowConfig::vertical),
            WireBP(050, 015, ElbowConfig::vertical),
            WireBP(050, 016, ElbowConfig::vertical),
            WireBP(050, 017, ElbowConfig::vertical),

            WireBP(051, 030, ElbowConfig::diagonalB),
            WireBP(051, 031, ElbowConfig::diagonalB),
            WireBP(051, 032, ElbowConfig::diagonalB),
            WireBP(051, 033, ElbowConfig::diagonalB),
            WireBP(051, 034, ElbowConfig::diagonalB),
            WireBP(051, 035, ElbowConfig::diagonalB),
            WireBP(051, 036, ElbowConfig::diagonalB),
            WireBP(051, 037, ElbowConfig::diagonalB),
        }),


    Blueprint(
        "4-to-1 Byte Multiplexer (MUX)",
        {
            NodeBP("A0", true, Gate::OR, IVec2(0,000) * g_gridSize),
            NodeBP("A1", true, Gate::OR, IVec2(0,001) * g_gridSize),
            NodeBP("A2", true, Gate::OR, IVec2(0,002) * g_gridSize),
            NodeBP("A3", true, Gate::OR, IVec2(0,003) * g_gridSize),
            NodeBP("A4", true, Gate::OR, IVec2(0,004) * g_gridSize),
            NodeBP("A5", true, Gate::OR, IVec2(0,005) * g_gridSize),
            NodeBP("A6", true, Gate::OR, IVec2(0,006) * g_gridSize),
            NodeBP("A7", true, Gate::OR, IVec2(0,007) * g_gridSize),

            NodeBP(    false, Gate::AND, IVec2(1,000) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,001) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,002) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,003) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,004) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,005) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,006) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,007) * g_gridSize),

            NodeBP("B0", true, Gate::OR, IVec2(0,010) * g_gridSize),
            NodeBP("B1", true, Gate::OR, IVec2(0,011) * g_gridSize),
            NodeBP("B2", true, Gate::OR, IVec2(0,012) * g_gridSize),
            NodeBP("B3", true, Gate::OR, IVec2(0,013) * g_gridSize),
            NodeBP("B4", true, Gate::OR, IVec2(0,014) * g_gridSize),
            NodeBP("B5", true, Gate::OR, IVec2(0,015) * g_gridSize),
            NodeBP("B6", true, Gate::OR, IVec2(0,016) * g_gridSize),
            NodeBP("B7", true, Gate::OR, IVec2(0,017) * g_gridSize),

            NodeBP(    false, Gate::AND, IVec2(1,010) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,011) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,012) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,013) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,014) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,015) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,016) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,017) * g_gridSize),

            NodeBP("C0", true, Gate::OR, IVec2(0,020) * g_gridSize),
            NodeBP("C1", true, Gate::OR, IVec2(0,021) * g_gridSize),
            NodeBP("C2", true, Gate::OR, IVec2(0,022) * g_gridSize),
            NodeBP("C3", true, Gate::OR, IVec2(0,023) * g_gridSize),
            NodeBP("C4", true, Gate::OR, IVec2(0,024) * g_gridSize),
            NodeBP("C5", true, Gate::OR, IVec2(0,025) * g_gridSize),
            NodeBP("C6", true, Gate::OR, IVec2(0,026) * g_gridSize),
            NodeBP("C7", true, Gate::OR, IVec2(0,027) * g_gridSize),

            NodeBP(    false, Gate::AND, IVec2(1,020) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,021) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,022) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,023) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,024) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,025) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,026) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,027) * g_gridSize),

            NodeBP("D0", true, Gate::OR, IVec2(0,030) * g_gridSize),
            NodeBP("D1", true, Gate::OR, IVec2(0,031) * g_gridSize),
            NodeBP("D2", true, Gate::OR, IVec2(0,032) * g_gridSize),
            NodeBP("D3", true, Gate::OR, IVec2(0,033) * g_gridSize),
            NodeBP("D4", true, Gate::OR, IVec2(0,034) * g_gridSize),
            NodeBP("D5", true, Gate::OR, IVec2(0,035) * g_gridSize),
            NodeBP("D6", true, Gate::OR, IVec2(0,036) * g_gridSize),
            NodeBP("D7", true, Gate::OR, IVec2(0,037) * g_gridSize),

            NodeBP(    false, Gate::AND, IVec2(1,030) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,031) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,032) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,033) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,034) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,035) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,036) * g_gridSize),
            NodeBP(    false, Gate::AND, IVec2(1,037) * g_gridSize),

            NodeBP("O0", true, Gate::OR, IVec2(2,014) * g_gridSize),
            NodeBP("O1", true, Gate::OR, IVec2(2,015) * g_gridSize),
            NodeBP("O2", true, Gate::OR, IVec2(2,016) * g_gridSize),
            NodeBP("O3", true, Gate::OR, IVec2(2,017) * g_gridSize),
            NodeBP("O4", true, Gate::OR, IVec2(2,020) * g_gridSize),
            NodeBP("O5", true, Gate::OR, IVec2(2,021) * g_gridSize),
            NodeBP("O6", true, Gate::OR, IVec2(2,022) * g_gridSize),
            NodeBP("O7", true, Gate::OR, IVec2(2,023) * g_gridSize),

            NodeBP("ID0", true, Gate::OR, IVec2(0,041) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(0,040)* g_gridSize),
            NodeBP("ID1", true, Gate::OR, IVec2(1,041) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(1,040) * g_gridSize),
        },
        {
            WireBP(000, 010, ElbowConfig::horizontal),
            WireBP(001, 011, ElbowConfig::horizontal),
            WireBP(002, 012, ElbowConfig::horizontal),
            WireBP(003, 013, ElbowConfig::horizontal),
            WireBP(004, 014, ElbowConfig::horizontal),
            WireBP(005, 015, ElbowConfig::horizontal),
            WireBP(006, 016, ElbowConfig::horizontal),
            WireBP(007, 017, ElbowConfig::horizontal),

            WireBP(020, 030, ElbowConfig::horizontal),
            WireBP(021, 031, ElbowConfig::horizontal),
            WireBP(022, 032, ElbowConfig::horizontal),
            WireBP(023, 033, ElbowConfig::horizontal),
            WireBP(024, 034, ElbowConfig::horizontal),
            WireBP(025, 035, ElbowConfig::horizontal),
            WireBP(026, 036, ElbowConfig::horizontal),
            WireBP(027, 037, ElbowConfig::horizontal),

            WireBP(040, 050, ElbowConfig::horizontal),
            WireBP(041, 051, ElbowConfig::horizontal),
            WireBP(042, 052, ElbowConfig::horizontal),
            WireBP(043, 053, ElbowConfig::horizontal),
            WireBP(044, 054, ElbowConfig::horizontal),
            WireBP(045, 055, ElbowConfig::horizontal),
            WireBP(046, 056, ElbowConfig::horizontal),
            WireBP(047, 057, ElbowConfig::horizontal),

            WireBP(060, 070, ElbowConfig::horizontal),
            WireBP(061, 071, ElbowConfig::horizontal),
            WireBP(062, 072, ElbowConfig::horizontal),
            WireBP(063, 073, ElbowConfig::horizontal),
            WireBP(064, 074, ElbowConfig::horizontal),
            WireBP(065, 075, ElbowConfig::horizontal),
            WireBP(066, 076, ElbowConfig::horizontal),
            WireBP(067, 077, ElbowConfig::horizontal),

            WireBP(010, 0100, ElbowConfig::diagonalA),
            WireBP(011, 0101, ElbowConfig::diagonalA),
            WireBP(012, 0102, ElbowConfig::diagonalA),
            WireBP(013, 0103, ElbowConfig::diagonalA),
            WireBP(014, 0104, ElbowConfig::diagonalA),
            WireBP(015, 0105, ElbowConfig::diagonalA),
            WireBP(016, 0106, ElbowConfig::diagonalA),
            WireBP(017, 0107, ElbowConfig::diagonalA),

            WireBP(030, 0100, ElbowConfig::diagonalA),
            WireBP(031, 0101, ElbowConfig::diagonalA),
            WireBP(032, 0102, ElbowConfig::diagonalA),
            WireBP(033, 0103, ElbowConfig::diagonalA),
            WireBP(034, 0104, ElbowConfig::diagonalA),
            WireBP(035, 0105, ElbowConfig::diagonalA),
            WireBP(036, 0106, ElbowConfig::diagonalA),
            WireBP(037, 0107, ElbowConfig::diagonalA),

            WireBP(050, 0100, ElbowConfig::diagonalA),
            WireBP(051, 0101, ElbowConfig::diagonalA),
            WireBP(052, 0102, ElbowConfig::diagonalA),
            WireBP(053, 0103, ElbowConfig::diagonalA),
            WireBP(054, 0104, ElbowConfig::diagonalA),
            WireBP(055, 0105, ElbowConfig::diagonalA),
            WireBP(056, 0106, ElbowConfig::diagonalA),
            WireBP(057, 0107, ElbowConfig::diagonalA),

            WireBP(070, 0100, ElbowConfig::diagonalA),
            WireBP(071, 0101, ElbowConfig::diagonalA),
            WireBP(072, 0102, ElbowConfig::diagonalA),
            WireBP(073, 0103, ElbowConfig::diagonalA),
            WireBP(074, 0104, ElbowConfig::diagonalA),
            WireBP(075, 0105, ElbowConfig::diagonalA),
            WireBP(076, 0106, ElbowConfig::diagonalA),
            WireBP(077, 0107, ElbowConfig::diagonalA),

            WireBP(0110, 0111, ElbowConfig::vertical),
            WireBP(0112, 0113, ElbowConfig::vertical),

            WireBP(0110, 010, ElbowConfig::vertical),
            WireBP(0110, 011, ElbowConfig::vertical),
            WireBP(0110, 012, ElbowConfig::vertical),
            WireBP(0110, 013, ElbowConfig::vertical),
            WireBP(0110, 014, ElbowConfig::vertical),
            WireBP(0110, 015, ElbowConfig::vertical),
            WireBP(0110, 016, ElbowConfig::vertical),
            WireBP(0110, 017, ElbowConfig::vertical),

            WireBP(0111, 030, ElbowConfig::diagonalB),
            WireBP(0111, 031, ElbowConfig::diagonalB),
            WireBP(0111, 032, ElbowConfig::diagonalB),
            WireBP(0111, 033, ElbowConfig::diagonalB),
            WireBP(0111, 034, ElbowConfig::diagonalB),
            WireBP(0111, 035, ElbowConfig::diagonalB),
            WireBP(0111, 036, ElbowConfig::diagonalB),
            WireBP(0111, 037, ElbowConfig::diagonalB),

            WireBP(0110, 050, ElbowConfig::vertical),
            WireBP(0110, 051, ElbowConfig::vertical),
            WireBP(0110, 052, ElbowConfig::vertical),
            WireBP(0110, 053, ElbowConfig::vertical),
            WireBP(0110, 054, ElbowConfig::vertical),
            WireBP(0110, 055, ElbowConfig::vertical),
            WireBP(0110, 056, ElbowConfig::vertical),
            WireBP(0110, 057, ElbowConfig::vertical),

            WireBP(0111, 070, ElbowConfig::diagonalB),
            WireBP(0111, 071, ElbowConfig::diagonalB),
            WireBP(0111, 072, ElbowConfig::diagonalB),
            WireBP(0111, 073, ElbowConfig::diagonalB),
            WireBP(0111, 074, ElbowConfig::diagonalB),
            WireBP(0111, 075, ElbowConfig::diagonalB),
            WireBP(0111, 076, ElbowConfig::diagonalB),
            WireBP(0111, 077, ElbowConfig::diagonalB),

            WireBP(0112, 010, ElbowConfig::vertical),
            WireBP(0112, 011, ElbowConfig::vertical),
            WireBP(0112, 012, ElbowConfig::vertical),
            WireBP(0112, 013, ElbowConfig::vertical),
            WireBP(0112, 014, ElbowConfig::vertical),
            WireBP(0112, 015, ElbowConfig::vertical),
            WireBP(0112, 016, ElbowConfig::vertical),
            WireBP(0112, 017, ElbowConfig::vertical),

            WireBP(0112, 030, ElbowConfig::diagonalB),
            WireBP(0112, 031, ElbowConfig::diagonalB),
            WireBP(0112, 032, ElbowConfig::diagonalB),
            WireBP(0112, 033, ElbowConfig::diagonalB),
            WireBP(0112, 034, ElbowConfig::diagonalB),
            WireBP(0112, 035, ElbowConfig::diagonalB),
            WireBP(0112, 036, ElbowConfig::diagonalB),
            WireBP(0112, 037, ElbowConfig::diagonalB),

            WireBP(0113, 050, ElbowConfig::vertical),
            WireBP(0113, 051, ElbowConfig::vertical),
            WireBP(0113, 052, ElbowConfig::vertical),
            WireBP(0113, 053, ElbowConfig::vertical),
            WireBP(0113, 054, ElbowConfig::vertical),
            WireBP(0113, 055, ElbowConfig::vertical),
            WireBP(0113, 056, ElbowConfig::vertical),
            WireBP(0113, 057, ElbowConfig::vertical),

            WireBP(0113, 070, ElbowConfig::diagonalB),
            WireBP(0113, 071, ElbowConfig::diagonalB),
            WireBP(0113, 072, ElbowConfig::diagonalB),
            WireBP(0113, 073, ElbowConfig::diagonalB),
            WireBP(0113, 074, ElbowConfig::diagonalB),
            WireBP(0113, 075, ElbowConfig::diagonalB),
            WireBP(0113, 076, ElbowConfig::diagonalB),
            WireBP(0113, 077, ElbowConfig::diagonalB),
        }),

    Blueprint(
        "1-Bit Half-Adder",
        {
            NodeBP("B", true, Gate::OR,  IVec2(0,1)* g_gridSize),
            NodeBP("A", true, Gate::OR,  IVec2(0,0)* g_gridSize),
            NodeBP("Sum", true, Gate::XOR, IVec2(1,0)* g_gridSize),
            NodeBP("Carry", true, Gate::AND, IVec2(1,1)* g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::horizontal),
            WireBP(1, 2, ElbowConfig::horizontal),
            WireBP(0, 3, ElbowConfig::vertical),
            WireBP(1, 3, ElbowConfig::vertical),
        }),

    Blueprint(
        "1-Bit Half-Subtractor",
        {
            NodeBP("A", true,  Gate::OR,  IVec2(0,0)* g_gridSize),
            NodeBP("B", true,  Gate::OR,  IVec2(0,1)* g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(1,1) * g_gridSize),
            NodeBP("Difference", true,  Gate::XOR, IVec2(2,0) * g_gridSize),
            NodeBP("Borrow", true,  Gate::AND, IVec2(2,1) * g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::vertical),
            WireBP(0, 3, ElbowConfig::horizontal),
            WireBP(1, 3, ElbowConfig::horizontal),
            WireBP(1, 4, ElbowConfig::vertical),
            WireBP(2, 4, ElbowConfig::vertical),
        }),

    Blueprint(
        "1-Bit Full-Adder",
        {
            NodeBP("A", true,  Gate::OR,  IVec2(0,0)* g_gridSize),
            NodeBP("B", true,  Gate::OR,  IVec2(0,1)* g_gridSize),
            NodeBP("Carry In", true,  Gate::OR,  IVec2(2,0)* g_gridSize),
            NodeBP(false, Gate::XOR, IVec2(1,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(1,1) * g_gridSize),
            NodeBP("Sum", true,  Gate::XOR, IVec2(3,0)* g_gridSize),
            NodeBP(false, Gate::AND, IVec2(2,1) * g_gridSize),
            NodeBP("Carry Out", true,  Gate::OR,  IVec2(3,1)* g_gridSize),
        },
        {
            WireBP(0, 3, ElbowConfig::horizontal),
            WireBP(1, 3, ElbowConfig::horizontal),
            WireBP(0, 4, ElbowConfig::vertical),
            WireBP(1, 4, ElbowConfig::vertical),
            WireBP(2, 5, ElbowConfig::horizontal),
            WireBP(3, 5, ElbowConfig::horizontal),
            WireBP(2, 6, ElbowConfig::vertical),
            WireBP(3, 6, ElbowConfig::vertical),
            WireBP(6, 7, ElbowConfig::vertical),
            WireBP(4, 7, ElbowConfig::horizontal),
        }),

    Blueprint(
        "1-Bit Full-Subtractor",
        {
            NodeBP("A", true,  Gate::OR,  IVec2(0,0)* g_gridSize),
            NodeBP("B", true,  Gate::OR,  IVec2(0,1)* g_gridSize),
            NodeBP("Borrow In", true,  Gate::OR,  IVec2(1,0)* g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(1,1) * g_gridSize),
            NodeBP(false, Gate::XOR, IVec2(2,0) * g_gridSize),
            NodeBP(false, Gate::AND, IVec2(2,1) * g_gridSize),
            NodeBP("Difference", true,  Gate::XOR, IVec2(4,0)* g_gridSize),
            NodeBP(false, Gate::AND, IVec2(3,1) * g_gridSize),
            NodeBP(false, Gate::NOR, IVec2(3,0) * g_gridSize),
            NodeBP("Borrow Out", true,  Gate::OR,  IVec2(4,1)* g_gridSize),
        },
        {
            WireBP(0, 3, ElbowConfig::vertical),
            WireBP(1, 5, ElbowConfig::vertical),
            WireBP(3, 5, ElbowConfig::vertical),
            WireBP(2, 6, ElbowConfig::vertical),
            WireBP(2, 7, ElbowConfig::vertical),
            WireBP(8, 7, ElbowConfig::vertical),
            WireBP(1, 4, ElbowConfig::vertical),
            WireBP(0, 4, ElbowConfig::vertical),
            WireBP(4, 8, ElbowConfig::vertical),
            WireBP(4, 6, ElbowConfig::vertical),
            WireBP(5, 9, ElbowConfig::vertical),
            WireBP(7, 9, ElbowConfig::vertical),
        }),

    Blueprint(
        "2-Bit Multiplier",
        {
            NodeBP("A0", true,  Gate::OR,  IVec2(0,0) * g_gridSize),
            NodeBP("A1", true,  Gate::OR,  IVec2(0,1) * g_gridSize),
            NodeBP("B0", true,  Gate::OR,  IVec2(0,2) * g_gridSize),
            NodeBP("B1", true,  Gate::OR,  IVec2(0,3) * g_gridSize),
            NodeBP(false,  Gate::AND,  IVec2(1,0) * g_gridSize),
            NodeBP(false,  Gate::AND,  IVec2(1,1) * g_gridSize),
            NodeBP(false,  Gate::AND,  IVec2(1,2) * g_gridSize),
            NodeBP(false,  Gate::AND,  IVec2(1,3) * g_gridSize),
            NodeBP(false,  Gate::XOR,  IVec2(2,0) * g_gridSize),
            NodeBP(false,  Gate::AND,  IVec2(2,1) * g_gridSize),
            NodeBP(false,  Gate::XOR,  IVec2(2,2) * g_gridSize),
            NodeBP(false,  Gate::AND,  IVec2(2,3) * g_gridSize),
            NodeBP("C0", true,  Gate::OR,  IVec2(3,0)* g_gridSize),
            NodeBP("C1", true,  Gate::OR,  IVec2(3,1)* g_gridSize),
            NodeBP("C2", true,  Gate::OR,  IVec2(3,2)* g_gridSize),
            NodeBP("C3", true,  Gate::OR,  IVec2(3,3)* g_gridSize),
        },
        {
            WireBP(0, 4, ElbowConfig::diagonalA),
            WireBP(0, 6, ElbowConfig::diagonalA),
            WireBP(1, 5, ElbowConfig::diagonalA),
            WireBP(1, 7, ElbowConfig::diagonalA),
            WireBP(2, 4, ElbowConfig::diagonalA),
            WireBP(2, 5, ElbowConfig::diagonalA),
            WireBP(3, 6, ElbowConfig::diagonalA),
            WireBP(3, 7, ElbowConfig::diagonalA),

            WireBP(4, 12, ElbowConfig::horizontal),
            WireBP(5, 8, ElbowConfig::horizontal),
            WireBP(6, 9, ElbowConfig::horizontal),
            WireBP(8, 13, ElbowConfig::horizontal),
            WireBP(9, 10, ElbowConfig::horizontal),
            WireBP(7, 11, ElbowConfig::horizontal),
            WireBP(10, 14, ElbowConfig::horizontal),
            WireBP(11, 15, ElbowConfig::horizontal),
        }),

    Blueprint(
        "Bit Comparator",
        {
            NodeBP("A",        true, Gate::OR,  IVec2(0,0) * g_gridSize),
            NodeBP("B",        true, Gate::OR,  IVec2(0,2) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,0) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,2) * g_gridSize),
            NodeBP("C (A<B)", false, Gate::AND, IVec2(2,0) * g_gridSize),
            NodeBP("E (A>B)", false, Gate::AND, IVec2(2,2) * g_gridSize),
            NodeBP(            true, Gate::XOR, IVec2(3,1) * g_gridSize),
            NodeBP("D (A=B)",  true, Gate::NOR, IVec2(4,1) * g_gridSize),
        },
        {
            WireBP(0, 2, ElbowConfig::horizontal),
            WireBP(2, 4, ElbowConfig::vertical),
            WireBP(0, 5, ElbowConfig::diagonalA),
            WireBP(5, 6, ElbowConfig::diagonalA),

            WireBP(1, 3, ElbowConfig::horizontal),
            WireBP(3, 5, ElbowConfig::vertical),
            WireBP(1, 4, ElbowConfig::diagonalA),
            WireBP(4, 6, ElbowConfig::diagonalA),

            WireBP(6, 7, ElbowConfig::horizontal),
        }),

    Blueprint(
        "Byte Comparator",
        {
            NodeBP("A1",       true, Gate::OR,  IVec2(0,000) * g_gridSize),
            NodeBP("A2",       true, Gate::OR,  IVec2(0,001) * g_gridSize),
            NodeBP("A3",       true, Gate::OR,  IVec2(0,002) * g_gridSize),
            NodeBP("A4",       true, Gate::OR,  IVec2(0,003) * g_gridSize),
            NodeBP("A5",       true, Gate::OR,  IVec2(0,004) * g_gridSize),
            NodeBP("A6",       true, Gate::OR,  IVec2(0,005) * g_gridSize),
            NodeBP("A7",       true, Gate::OR,  IVec2(0,006) * g_gridSize),
            NodeBP("A8",       true, Gate::OR,  IVec2(0,007) * g_gridSize),

            NodeBP(           false, Gate::NOR, IVec2(1,000) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,001) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,002) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,003) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,004) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,005) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,006) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,007) * g_gridSize),

            NodeBP(           false, Gate::AND, IVec2(2,000) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,001) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,002) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,003) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,004) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,005) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,006) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,007) * g_gridSize),

            NodeBP("B1",       true, Gate::OR,  IVec2(0,010) * g_gridSize),
            NodeBP("B2",       true, Gate::OR,  IVec2(0,011) * g_gridSize),
            NodeBP("B3",       true, Gate::OR,  IVec2(0,012) * g_gridSize),
            NodeBP("B4",       true, Gate::OR,  IVec2(0,013) * g_gridSize),
            NodeBP("B5",       true, Gate::OR,  IVec2(0,014) * g_gridSize),
            NodeBP("B6",       true, Gate::OR,  IVec2(0,015) * g_gridSize),
            NodeBP("B7",       true, Gate::OR,  IVec2(0,016) * g_gridSize),
            NodeBP("B8",       true, Gate::OR,  IVec2(0,017) * g_gridSize),

            NodeBP(           false, Gate::NOR, IVec2(1,010) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,011) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,012) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,013) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,014) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,015) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,016) * g_gridSize),
            NodeBP(           false, Gate::NOR, IVec2(1,017) * g_gridSize),

            NodeBP(           false, Gate::AND, IVec2(2,010) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,011) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,012) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,013) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,014) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,015) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,016) * g_gridSize),
            NodeBP(           false, Gate::AND, IVec2(2,017) * g_gridSize),

            NodeBP("C (A<B)",  true, Gate::OR,  IVec2(3,006) * g_gridSize),
            NodeBP("D'(A!=B)", true, Gate::XOR, IVec2(3,007) * g_gridSize),
            NodeBP("D (A=B)",  true, Gate::NOR, IVec2(3,010) * g_gridSize),
            NodeBP("E (A>B)",  true, Gate::OR,  IVec2(3,011) * g_gridSize),
        },
        {
            WireBP(000, 010, ElbowConfig::horizontal),
            WireBP(001, 011, ElbowConfig::horizontal),
            WireBP(002, 012, ElbowConfig::horizontal),
            WireBP(003, 013, ElbowConfig::horizontal),
            WireBP(004, 014, ElbowConfig::horizontal),
            WireBP(005, 015, ElbowConfig::horizontal),
            WireBP(006, 016, ElbowConfig::horizontal),
            WireBP(007, 017, ElbowConfig::horizontal),

            WireBP(010, 020, ElbowConfig::horizontal),
            WireBP(011, 021, ElbowConfig::horizontal),
            WireBP(012, 022, ElbowConfig::horizontal),
            WireBP(013, 023, ElbowConfig::horizontal),
            WireBP(014, 024, ElbowConfig::horizontal),
            WireBP(015, 025, ElbowConfig::horizontal),
            WireBP(016, 026, ElbowConfig::horizontal),
            WireBP(017, 027, ElbowConfig::horizontal),

            WireBP(000, 050, ElbowConfig::diagonalA),
            WireBP(001, 051, ElbowConfig::diagonalA),
            WireBP(002, 052, ElbowConfig::diagonalA),
            WireBP(003, 053, ElbowConfig::diagonalA),
            WireBP(004, 054, ElbowConfig::diagonalA),
            WireBP(005, 055, ElbowConfig::diagonalA),
            WireBP(006, 056, ElbowConfig::diagonalA),
            WireBP(007, 057, ElbowConfig::diagonalA),

            WireBP(030, 040, ElbowConfig::horizontal),
            WireBP(031, 041, ElbowConfig::horizontal),
            WireBP(032, 042, ElbowConfig::horizontal),
            WireBP(033, 043, ElbowConfig::horizontal),
            WireBP(034, 044, ElbowConfig::horizontal),
            WireBP(035, 045, ElbowConfig::horizontal),
            WireBP(036, 046, ElbowConfig::horizontal),
            WireBP(037, 047, ElbowConfig::horizontal),

            WireBP(040, 050, ElbowConfig::horizontal),
            WireBP(041, 051, ElbowConfig::horizontal),
            WireBP(042, 052, ElbowConfig::horizontal),
            WireBP(043, 053, ElbowConfig::horizontal),
            WireBP(044, 054, ElbowConfig::horizontal),
            WireBP(045, 055, ElbowConfig::horizontal),
            WireBP(046, 056, ElbowConfig::horizontal),
            WireBP(047, 057, ElbowConfig::horizontal),

            WireBP(030, 020, ElbowConfig::diagonalA),
            WireBP(031, 021, ElbowConfig::diagonalA),
            WireBP(032, 022, ElbowConfig::diagonalA),
            WireBP(033, 023, ElbowConfig::diagonalA),
            WireBP(034, 024, ElbowConfig::diagonalA),
            WireBP(035, 025, ElbowConfig::diagonalA),
            WireBP(036, 026, ElbowConfig::diagonalA),
            WireBP(037, 027, ElbowConfig::diagonalA),

            WireBP(020, 060, ElbowConfig::diagonalB),
            WireBP(021, 060, ElbowConfig::diagonalB),
            WireBP(022, 060, ElbowConfig::diagonalB),
            WireBP(023, 060, ElbowConfig::diagonalB),
            WireBP(024, 060, ElbowConfig::diagonalB),
            WireBP(025, 060, ElbowConfig::diagonalB),
            WireBP(026, 060, ElbowConfig::diagonalB),
            WireBP(027, 060, ElbowConfig::diagonalB),

            WireBP(050, 063, ElbowConfig::diagonalB),
            WireBP(051, 063, ElbowConfig::diagonalB),
            WireBP(052, 063, ElbowConfig::diagonalB),
            WireBP(053, 063, ElbowConfig::diagonalB),
            WireBP(054, 063, ElbowConfig::diagonalB),
            WireBP(055, 063, ElbowConfig::diagonalB),
            WireBP(056, 063, ElbowConfig::diagonalB),
            WireBP(057, 063, ElbowConfig::diagonalB),

            WireBP(060, 061, ElbowConfig::vertical),
            WireBP(063, 061, ElbowConfig::vertical),
            WireBP(061, 062, ElbowConfig::vertical),
        }),
};
